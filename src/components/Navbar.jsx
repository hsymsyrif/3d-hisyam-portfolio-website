  ...
  return (
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 ${scrolled ? "bg-primary" : "bg-transparent"}`}>
      <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
        ...
        >
          <img src={logowhite} alt="logowhite" className="w-8 h-9 object-contain" />
        ...
        <ul className="list-none hidden sm:flex flex-row gap-10">
          {navLinks.map((nav) => (
            <li key={nav.id} className={`${active === nav.title ? "text-white" : "text-white"} hover:text-[#4ea0ed] text-[18px] font-medium cursor-pointer`} onClick={() => setActive(nav.title)}>
